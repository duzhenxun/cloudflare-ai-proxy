...
claudeRouter.post("/v1/messages", async (c) => {
    try {
        ...
        // Map OpenAI errors to Claude errors
        if (error.status) {
            status = error.status;
            if (error.error) {
                message = error.error.message || message;
                const openAIType = error.error.type;

                // Map OpenAI error types to Claude error types
                if (openAIType === 'invalid_request_error') type = 'invalid_request_error';
                else if (openAIType === 'authentication_error') type = 'authentication_error';
                else if (openAIType === 'permission_error') type = 'permission_error';
